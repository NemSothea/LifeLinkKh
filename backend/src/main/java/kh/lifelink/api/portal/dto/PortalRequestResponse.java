 ...
 * computed-on-read counts as the mobile contract's {@code BloodRequestResponse} — one meaning, read
 * by two clients, never two counters that can disagree.
 *
 ...
 *     list rather than one with a "confirm" button that would only 409. {@code acceptedCount} still
 *     counts every acceptance, confirmed or not — same meaning as the mobile contract's.
 */
