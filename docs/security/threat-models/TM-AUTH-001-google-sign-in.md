...
| E1 | Server-side allow-list: a self-service sign-up may only produce `DONOR` or `REQUESTER`. `HOSPITAL` and `ADMIN` are assignable only by an existing `ADMIN`. Reject, do not silently downgrade — silent downgrade hides the attempt. | M3 |
